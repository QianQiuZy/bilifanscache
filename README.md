...
服务启动后会持续轮询 `rooms.json` 中配置的所有主播 UID，拉取粉丝牌分页数据并写入 Redis（默认 DB1）与进程内缓存；同时提供按房间、按主播以及按用户维度的查询接口。

## 特性

- **全量轮询**：持续遍历 `rooms.json` 内全部房间，不再按分钟级批次等待。
- **固定频率**：每次请求 B 站接口后默认等待 3 秒，再发起下一次请求。
- **多维查询**：
...
Redis 默认配置如下（可在 `.env` 覆盖）：

- `REDIS_HOST=127.0.0.1`
- `REDIS_PORT=6379`
- `REDIS_DB=1`（按需避开 DB0）
- `REDIS_PASSWORD=`（默认空密码）
- `REDIS_KEY_PREFIX=bilifanscache`
...
- Redis 中没有缓存的房间会在启动阶段优先完成首次抓取，并写入内存与 Redis。
- 缺失房间补齐后，才开始持续轮询全部房间。
- 启动补齐完成前服务不会对外提供请求；运行中尚未初始化的房间仍会返回 `503`。

## 注意事项

- 需提供有效的 `SESSDATA`，否则会导致 B 站接口返回异常。
- Redis 需可用，否则服务启动会失败（防止进入无持久化运行状态）。
